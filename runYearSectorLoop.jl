...
for year in "scr" .* string.(collect(1982:2016))
    ...
    # input folders
    inDir_arr = [modDir_str * "basis", modDir_str * "infeasParameter", setupDir_str * scrDir_str, setupDir_str * "spatialScope/" * spaSco, setupDir_str * "techSetup/endogenous_heatAndTransport", setupDir_str * "resolution/default_country", modDir_str * "timeSeries/country_" * time * "_month/general"]
