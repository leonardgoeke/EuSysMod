...
dir_str = "inputFiles/timeSeries/"


for fore_str in  ["month"]

    if fore_str == "3month"
        fore_tup = ("ini01","ini02","ini03","ini04") 
    else
    ...
    for t in ["2856","8760"]
        ...
        #region # merge seperate solder files

        inFolder_str = "greenfield_ESCU_country_" * t  * "h_" * fore_str * "_onlySolar"

        wrtDir_str = dir_str * "country_" * t  * "h_" * fore_str

        # move potential files
        mv(dir_str * inFolder_str * "/par_potentialPvInd.csv", wrtDir_str * "/general/par_potentialPvInd.csv")
        mv(dir_str * inFolder_str * "/par_potentialPvOpenspace.csv", wrtDir_str * "/general/par_potentialPvOpenspace.csv")

        # move time-series files
        scr_arr = string.(1982:2016)
        for c in ("pvInd","pvOpenspace","pvResi")
            for s in scr_arr
                for f in fore_tup
                    file_str = "/par_" * c * "_scr" * string(s) * "_" * f * ".csv"
                    mv(dir_str * inFolder_str * file_str, wrtDir_str * "/scr" * string(s) * "/" * f  * file_str)
                end
            end
        end

        # remove old solar ava
        scr_arr = string.(1982:2016)
        for s in scr_arr
            for f in fore_tup
                file_str = "/par_solarAva_scr" * string(s) * "_" * f * ".csv"
                rm(wrtDir_str * "/scr" * string(s) * "/" * f  * file_str)
            end
        end

        #endregion

        #region # used to reorganize folder, AFTER creatint time-series with seperate scripts
