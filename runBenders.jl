...
heu = par_df[id_int,:heu] # method for heuristic pre-solve
...
inOos = "missing" # string(par_df[id_int,:inputOutOfSample]) # capacity folder for out-of-sample testing
...
scrQrtHeu_arr, scrDirHeu_str, ~ = generateScrInfo(false, heu * "_" * split(scenario,"_")[end], setupDir_str, "month")
...
rngVio_ntup = (stab = 2e2, cut = 1e1, fix = 1e3)
...
if interpolate
	noStab_tup = (upper = 40, inter = :log, sub = 2.0)
else
...
info_ntup = (name = name_str, frsLvl = foresight, decompLvl = decomp, supTsLvl = 2, repTsLvl = 4, shortExp = 5, infeasTop = !(infeasTop in ("none", "none_half1", "none_half2")))

# ! input folders
inDir_arr = [modDir_str * "basis", modDir_str * "infeasParameter", setupDir_str * "securitySetup/" * security, setupDir_str * "infeasTop/" * infeasTop, setupDir_str * "regionSetup/" * regionalScope, setupDir_str * "techSetup/" * techs, setupDir_str * "timeSetup/" * time * "/" * spLength, setupDir_str * "importCase/" * imp, setupDir_str * "resolution/" * reso, scrDir_str, modDir_str * "timeSeries/country_" * time * "/general"]
...
heuDir_arr = [modDir_str * "basis", modDir_str * "infeasParameter", setupDir_str * "securitySetup/" * security, setupDir_str * "infeasTop/" * infeasTop, setupDir_str * "regionSetup/" * regionalScope, setupDir_str * "techSetup/" * techs, setupDir_str * "timeSetup/672h/" * "month", setupDir_str * "importCase/" * imp, setupDir_str * "resolution/" * reso, scrDirHeu_str, modDir_str * "timeSeries/country_672h/general"]
...
scale_dic[:facTop] = (capa = 1e2, capaStSize = 1e4, insCapa = 1e2, dispConv = 1e3, dispSt = 1e4, dispExc = 1e3, dispTrd = 1e2, costDisp = 1e1, costCapa = 1e1, obj = 1e3)	
