...
decomp = convert(String,par_df[id_int,:decomp]) # decomp case
...
input_arr = [dir_str * "basis", dir_str * "spatialScope/" * spaSco, scrDir_str, dir_str * "timeSeries/setup/" * time * "h_" * (decomp == "year" ? "month" : decomp)]
foreach(x -> push!(input_arr, dir_str * "timeSeries/data/" * case * "_" * time * "h/" * x[1] * "/" * x[2]), scr_arr)

resultDir_str = dir_str * "results"

resData_df = DataFrame(case = Symbol[], variable = String[], value = Float64[])
name_str = "mono_" * time * "_" * spaSco * "_" * case * "_" * scr * "_" * string(foresight) * "_" * decomp
